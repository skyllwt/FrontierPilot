...
1. Use `scripts/search_arxiv.sh "<query>" [count] [sortBy]` to get the XML results. The script auto-filters to CS/ML categories (cs.LG, cs.AI, cs.CV, cs.CL, cs.NE, stat.ML). Default: 30 results sorted by lastUpdatedDate.
2. Parse the XML (look for `<entry>`, `<title>`, `<summary>`, `<link title="pdf">`, `<arxiv:comment>` for venue info, and `<arxiv:primary_category>`).
